...
| **`event.html`** | **이벤트** — 탭형 목록·상세 패널, **페이지로 이동** 버튼으로 인스타 등 링크 열기 |
| **`ggongbab.html`** | **꽁밥 안내** — 왼쪽 KAIST 캠퍼스 지도, 오른쪽 일정·행사 탭 |

상단 내비: 인스타그램 · 맛집 지도 · 먹방 가계부 · 카카오톡 · 유튜브 · 이벤트 · 언어(EN/한국어).  
...
- **환영 팝업** — **`index.html`(홈)** 에서만 표시. `food.html`, `event.html`, `ggongbab.html` 등에서는 뜨지 않습니다.  
...
- **이벤트 탭** — 카드 **왼쪽**을 누르면 상세 패널이 바뀝니다. **오른쪽** pill **페이지로 이동**으로 인스타 등 링크를 엽니다. 상세 본문 라벨 규칙은 **`docs/EVENT_DETAIL_FIELDS.md`** 참고.
...
2. 유튜브·카카오톡(내비에서 캐러셀 슬라이드로 이동)  
3. **먹방 가계부** (`food.html`)  
4. **이벤트** (`event.html`) — 탭·상세·인스타 연동  
5. 다국어(한/영) · 스크롤 진행 표시줄 · Apple 스타일 푸터(정책 링크는 자리만, URL은 필요 시 수정)
