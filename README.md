...
| **`event.html`** | **이벤트** — 탭형 목록(날짜·제목), 상세 패널, 첫 탭은 인스타 안내로 **새 창 연결** |
...
- **환영 팝업** — **`index.html`(홈)** 에서만 표시. `food.html`, `event.html`에서는 뜨지 않습니다.  
...
- **이벤트 첫 탭** — 인스타그램으로 **`target="_blank"`** 연결. 영상·게시물 URL이 정해지면 `event.html` 안 첫 탭의 **`href`** 만 교체하면 됩니다(주석 참고).
